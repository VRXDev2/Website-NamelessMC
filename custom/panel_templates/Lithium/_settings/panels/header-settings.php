<?php

/**
 *	LITHIUM TEMPLATE
 *	By Xemah | https://xemah.com
 *
**/

?>

<div class="lithium-panel-content">
	<div class="lithium-panel-header">
		<a href="#lithium-panel-menu">
			<i class="fas fa-angle-left"></i>
		</a>
		<div class="lithium-panel-title">
			Header Settings
		</div>
	</div>
	<div class="lithium-panel-body">
		<form>
			<?php
			
			createInput('enableHeader', 'switch', [
				'label' => 'Enable Header',
				'value' => '1'
			]);
			
			createInput('displayHeaderOnOnlyHomepage', 'switch', [
				'label' => 'Display header on only homepage',
				'value' => '0'
			]);
			
			createInput('headerBackground', 'image', [
				'label' => 'Header Background',
				'value' => ''
			]);
			
			createInput('headerHeight', 'textShort', [
				'label' => 'Header Height',
				'value' => '300px'
			]);

			createInput('headerFadeEffect', 'select', [
				'label' => 'Header Fade Effect',
				'value' => 'onlyOnDarkMode',
				'options' => [
					'off' => 'Off',
					'on' => 'On',
					'onlyOnLightMode' => "Only on light mode",
					'onlyOnDarkMode' => "Only on dark mode",
				]
			]);
			
			createInput('enableHeaderZoomOnScroll', 'switch', [
				'label' => 'Enable Header Zoom On Scroll Effect',
				'value' => '0'
			]);

			echo('<hr />');
			
			createInput('enableHeaderParticles', 'switch', [
				'label' => 'Enable Particles',
				'value' => '0'
			]);
			
			createInput('headerParticlesColor', 'color', [
				'label' => 'Particles Color',
				'value' => '#ffffff'
			]);

			echo('<hr />');

			createInput('headerLogo', 'image', [
				'label' => 'Logo',
				'value' => 'data:image/png;base64,'
			]);

			createInput('headerLogoHeight', 'textShort', [
				'label' => 'Logo Height',
				'value' => '200px'
			]);
			
			createInput('enableHeaderLogoAnimation', 'switch', [
				'label' => 'Enable Logo Animation',
				'value' => '1'
			]);

			echo('<hr />');
			
			createInput('displayMinecraftServer', 'switch', [
				'label' => 'Display Minecraft Server',
				'value' => '0'
			]);
			
			createInput('minecraftServerIcon', 'text', [
				'label' => 'Minecraft Server Icon',
				'value' => '<i class="fas fa-cube"></i>'
			]);
			
			createInput('minecraftServerIp', 'text', [
				'label' => 'Minecraft Server IP',
				'value' => ''
			]);

			echo('<hr />');
			
			createInput('displayDiscordServer', 'switch', [
				'label' => 'Display Discord Server',
				'value' => '0'
			]);
			
			createInput('discordServerIcon', 'text', [
				'label' => 'Discord Server Icon',
				'value' => '<i class="fab fa-discord"></i>'
			]);
			
			createInput('discordServerId', 'text', [
				'label' => 'Discord Server ID',
				'value' => ''
			]);
			
			createInput('discordServerInviteLink', 'text', [
				'label' => 'Discord Server Invite Link',
				'value' => ''
			]);
			
			?>

		</form>
	</div>
</div>